...
**內容行文與邏輯為主軸，格式規範為輔。** 在報告中優先呈現影響論述說服力的問題（論點缺依據、結構不完整、語意模糊），格式細節（數字書寫、符號規範）最後列出，且不應佔據報告的主要篇幅。
...
- **重複贅述**：同一意思前後重複表達
- **句子過長**：超過 40 字考慮拆分
- **數字書寫**：1–9 應拼寫（one、two...），10 以上用阿拉伯數字；句首數字一律拼寫
- **數字與單位間距**：數字與單位之間需有空格（`5 mg`、`10 mL`），百分比符號例外（`5%` 可接受）
- **數值範圍符號**：範圍應使用 en dash（–），不用連字號（-），如 `10–20 mg`
- **種族與族群用語**：使用目前公認用語（Asian、Black、White），避免過時用語（如 Oriental）
